...
## CRITICAL: manifest.json is GENERATED — never hand-edit it

`manifest.json` is a **generated artifact**, rebuilt deterministically by
`tools/manifest.py`. Editing it by hand or with free-form writes is what caused repeated
truncation/corruption. Do NOT write to `manifest.json` directly — ever.

**Source of truth** = each `companies/<slug>/company.json` (its metadata + its `documents`
records) plus the files on disk. The manifest is a pure function of those.
...
- Add/update a document:
  ...
  (edits the company.json, validates, then rebuilds — all atomic.)
- Or edit `companies/<slug>/company.json` by hand, then: `python3 tools/manifest.py rebuild`
- Global state (watchlist, `last_global_refresh`) lives in `_manifest_globals.json`.

### How to read / report status
- `python3 tools/manifest.py rebuild` prints company/doc/calendar counts and integrity warnings.
- `python3 tools/manifest.py validate` checks every company.json + the manifest; non-zero exit on error.
- Read `manifest.json` for: `reporting_calendar` (sorted, what's due next), per-company `documents`,
  `_untracked_files` / `_missing_files` (gaps), and `_integrity_warnings`.

Writes are atomic (temp -> reparse guard -> os.replace), validated against a schema, and
backed up to `.manifest-backups/`. A partial write can never land.

## Purpose

Track every company we cover, every document we've captured, when we last checked for
updates, and what's expected next — and surface gaps (untracked files, missing files,
overdue results) rather than hide them.

## Manifest Location

`manifest.json` at the research root (this Cowork folder). GENERATED by `tools/manifest.py`.
